% function MGR_map = maxGrowthRateMap(Ri_list,Pe_list,Lambda,Pr,tau,N)
...
%                 Pe=Pe_list(Pe_index);
%                 MGR_map(Ri_index,Pe_index) = findMaxGrowthRate(Ri,Pe,Lambda,Pr,tau,N);
%             end
%         end
%     else
...
function MGR_map = maxGrowthRateMap(Ri_list,Pe_list,Lambda,Pr,tau)
    sizeOfRi = size(Ri_list,1);
    sizeOfPe = size(Pe_list,1);
    ...
    if canUseParallelPool
        ...
        parfor index=0:(leng-1)
            ...
            MGR(index+1) = findMaxGrowthRate(Ri_list(Ri_index+1),Pe_list(Pe_index+1),Lambda,Pr,tau);
        end
    else
        % Parallel Computing Toolbox is not installed
        for index=0:(leng-1)
